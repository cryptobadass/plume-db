...
import (
	...
	"github.com/plume-protocol/plume-db/sc/memiavl"
	"github.com/plume-protocol/plume-db/sc/types"
)
...
func (cs *CommitStore) LoadVersion(targetVersion int64, copyExisting bool) (types.Committer, error) {
	cs.logger.Info(fmt.Sprintf("PlumeDB load target memIAVL version %d, copyExisting = %v\n", targetVersion, copyExisting))
	if copyExisting {
		opts := cs.opts
